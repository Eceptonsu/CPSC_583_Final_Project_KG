...
from models import CP, ComplEx, TransE, RESCAL, TuckER, UniBi_3, RotatE
from regularizers import F2, N3, DURA_UniBi_2, DURA_UniBi_3
...
def setup_model(opt):
    ...
    elif opt['model'] == 'RotatE':
        model = RotatE(opt['size'], opt['rank'], opt['init'], opt['regularizer'])
    elif opt['model'] == 'UniBi_3':
        model = UniBi_3(opt['size'], opt['rank'], opt['init'], opt['rel_norm'], opt['ent_norm'], opt['regularizer'])
...
def setup_regularizer(opt):
    ...
    elif opt['regularizer'] == 'DURA_UniBi_2':
        regularizer = DURA_UniBi_2(opt['lmbda'])
    elif opt['regularizer'] == 'DURA_UniBi_3':
        regularizer = DURA_UniBi_3(opt['lmbda'])
    return regularizer

def setup_scheduler(opt, optimizer, total_step):
    if opt['set_scheduler'] == "linear":
        return get_linear_schedule_with_warmup(optimizer, num_warmup_steps=int(total_step // 2 * 0.05), num_training_steps=total_step)
    elif opt['set_scheduler'] == 'cosine':
        return get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=int(total_step // 2 * 0.05), num_training_steps=total_step)
...
class KBCEngine(object):
    ...
    def episode(self):
        ...
        length = exp_train_sampler.size
        single_epoch_step = length // self.batch_size if length % self.batch_size == 0 else (length // self.batch_size) + 1
        total_step = single_epoch_step * self.max_epochs * 2
        if self.opt['set_scheduler'] != None:
            self.scheduler = setup_scheduler(self.opt, self.optimizer, total_step)
        ...
        for e in range(init_epoch_id, self.max_epochs):
            ...
            if e % self.opt['valid'] == 0:
                ...
                for split in self.dataset.splits:
                    ...
                    # wandb.log({'step_wise/{}/hits@3'.format(split): res[split]['hits@[1,3,10]'][1]}, step=step_idx)
                    # wandb.log({'step_wise/{}/hits@3'.format(split): res[split]['hits@[1,3,10]'][1]}, step=step_idx)
